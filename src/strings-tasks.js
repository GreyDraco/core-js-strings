...
function isPalindrome(str) {
  ...
  for (let i = 0; i < clearstr.length / 2; i += 1) {
    if (clearstr.charAt(i) !== clearstr.charAt(clearstr.length - 1 - i)) {
      return false;
    }
  }
  return true;
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i <= wordArray.length - 1; i += 1) {
    if (wordArray[i].length > longest.length) longest = wordArray[i];
  }
  return longest;
}
...
function invertCase(str) {
  ...
  for (let i = 0; i <= charArray.length - 1; i += 1) {
